"""
Модуль для управления учетом расходов и бюджетом с использованием базы данных SQLite.
Включает классы и функции для работы с базой данных, создания интерфейса и
взаимодействия с пользователем для управления категориями, расходами и бюджетом.
"""
...
class AbstractView(Protocol):
    ...
    def register_handlers(self,
                          handlers: dict[str, list[Optional[Callable]]] | None = None):
        pass
...
class Bookkeeper:
    ...
    def edit_expenses(self, pk: int, amount: float, category: str,
                      expense_date: datetime, comment: str) -> None:
    ...
    def add_expense(self, amount: float, date: datetime,
                    category: str, comment: str) -> None:
        self.expenses_repo.add(Expense(amount=amount, category=category,
    ...
    def get_budget(self) -> list[Budget]:
        day_budgets = self.budget_repo.get_all(
            subquery="WHERE expiration_date=(SELECT MAX(expiration_date) \
                FROM budget WHERE duration='День')")
        week_budgets = self.budget_repo.get_all(
            subquery="WHERE expiration_date=(SELECT MAX(expiration_date) \
                FROM budget WHERE duration='Неделя')")
        month_budgets = self.budget_repo.get_all(
            subquery="WHERE expiration_date=(SELECT MAX(expiration_date) \
                FROM budget WHERE duration='Месяц')")
    ...
    def get_expenses_from_data_range(
        ...
        expenses = self.expenses_repo.get_all(
            subquery=f"WHERE expense_date <= '{end_date_str}' \
                AND expense_date > '{start_date_str}'"
        )
        return expenses
    ...
    def set_budget(self, amount: float, duration: str) -> None:
        ...
        budget = Budget(amount=start_amount, limits=amount, duration=duration,
